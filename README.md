...
**타이머의 동작을 위한 REST요청과 응답**

* 타이머는 REST서버를 통해 /start, /stop을 통해 제어할 수 있습니다. 그러한 요청의 응답으로 active, running 상태를 확인할 수 있습니다.

* 타이머는 반복하는 주기마다 삭제를 요구할 수 있습니다.

* 타이머가 1분(운영변수)마다 반복하여 삭제를 요구할 것입니다.

* 타이머는 비동기로 동작을 하기 때문에 1분(운영변수) 후에도 아직 삭제를 하는 중이면 삭제를 요구하지 않고 지나갑니다.

**삭제의 동작**

* 감시할 경로가 복잡하고 파일의 수가 많을 수록 삭제하는 시간이 길어질 것입니다. 그러한 과정은 예상보다 오래 걸릴 수가 있고 CPU점유율을 높일 수가 있습니다.

* 감시하는 경로에서 디렉토리(폴더) 목록을 읽습니다.

* 디렉토리에 있을 파일의 수가 지정한 파일수(운영변수) 보다 많은 경우에 실제로 삭제를 시작합니다. 디렉토리 마다 파일의 수를 확인하는 것은 단순하지만 파일의 수가 많으면 예상보다 CPU점유율을 높일 수가 있습니다.

* 디렉토리에 있을 전체 파일 목록을 읽습니다. 그리고 수정일시(modified time)을 활용해 다시 정렬(sort)한 목록을 만듭니다. 지정한 파일수 이상은 이제 실제로 파일을 삭제할 것입니다.

* 삭제한 파일 정보와 삭제에 걸린 시간을 출력합니다. 도커에서 실행을 한 경우에 docker logs -t -f dcache 명령어를 통해 확인을 할 수 있을 것입니다.
